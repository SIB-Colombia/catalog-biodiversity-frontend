	...
	async.parallel(
		{
			initialRecordsSpeciesData: function(callback) {
				request({
					url: appConfigVars.backendURL+'/index.php/api/fichasresumenhumedal?page=1&order=scientificname&orderdirection=asc&priorityimages',
					method: 'GET',
					json: true
			...
			initialRecordsSpeciesDataRandomWithImages: function(callback) {
				request({
					url: appConfigVars.backendURL+'/index.php/api/fichas/previewrandomspecieshumedal',
					method: 'GET',
					json: true
				}, function(error, response, body) {
					if (!error && response.statusCode == 200) {
